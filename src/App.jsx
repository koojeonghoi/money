...
import { Upload, X, Plus, RotateCcw, Loader2, ImagePlus, Trash2, PiggyBank } from "lucide-react";
...
async function extractTransactions(base64, mediaType, categoryNames) {
  const response = await fetch("/api/analyze", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ image: base64, mediaType, categories: categoryNames })
  });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        localStorage.setItem("ledger-data", JSON.stringify({ categories, transactions, income }));
      } catch (e) {
        console.error("저장 실패", e);
      }
    }, 400);
    return () => clearTimeout(saveTimer.current);
  }, [categories, transactions, income, loaded]);
  ...
  const processImageFile = useCallback(async (file) => {
    ...
    try {
      ...
      const rows = await extractTransactions(base64, mediaType, categoryNames);
      if (rows.length === 0) {
        ...
        const newTx = rows.map((r) => {
          const matched = findCategoryByName(r.category);
          return {
            id: uid(),
            date: r.date || "",
            description: r.description || "내역 없음",
            amount: Number(r.amount) || 0,
            categoryId: matched ? matched.id : uncategorized
          };
        });
  ...
  const addCategory = () => {
    ...
    setCategories((prev) => {
      const uncatIdx = prev.findIndex((c) => c.id === "uncategorized");
      const next = [...prev];
      next.splice(uncatIdx === -1 ? prev.length : uncatIdx, 0, newCat);
      return next;
    });
  ...
  const totalsByCategory = useMemo(() => {
    return categories
      .map((c) => ({
        ...c,
        total: transactions.filter((t) => t.categoryId === c.id).reduce((s, t) => s + Number(t.amount || 0), 0)
      }))
      .filter((c) => c.total !== 0);
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: "#101B2D", color: "#EDE6D3", fontFamily: "'Noto Sans KR','Malgun Gothic',sans-serif" }}>
      <style>{`
        ...
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm mt-0.5" style={{ color: "#93A0B8" }}>
              내역을 캡처해서 붙여넣으면 자동으로 분류합니다 · 고정지출 · 변동지출 · 저축을 한눈에
            </p>
          </div>
        </div>
      </header>

      <main className="max-w-5xl mx-auto px-5 md:px-10 py-6 grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          ...
          {/* Income */}
          <div className="rounded-2xl p-4" style={{ background: "#16233A", border: "1px solid #2A3B57" }}>
          ...
          {/* Category manager */}
          <div className="rounded-2xl p-4" style={{ background: "#16233A", border: "1px solid #2A3B57" }}>
            <button onClick={() => setShowCatManager((v) => !v)} className="w-full flex items-center justify-between text-sm font-semibold">
              <span>🏷️ 카테고리 편집 ({categories.length}개)</span>
              <span style={{ color: "#93A0B8" }}>{showCatManager ? "접기 ▲" : "펼치기 ▼"}</span>
            </button>
            {showCatManager && (
                    ...
                    <div className="flex gap-0.5 flex-shrink-0">
                      {PALETTE.slice(0, 6).map((c) => (
                        <button
                          ...
                          style={{ background: c, outline: cat.color === c ? "1.5px solid #EDE6D3" : "none", outlineOffset: 1 }}
                        />
                      ))}
                    </div>
        ...
        {/* RIGHT: ledger table & summary */}
        <div className="lg:col-span-3 flex flex-col gap-5">
          {/* Summary cards */}
          <div className="grid grid-cols-3 gap-3">
            {totalsByType.map((t) => (
              <div key={t.type} className="rounded-2xl p-3 text-center" style={{ background: "#16233A", border: "1px solid #2A3B57" }}>
          ...
          {(fixedRatio !== null || savingRatio !== null) && (
            <div className="rounded-2xl p-4 flex flex-wrap gap-x-6 gap-y-2" style={{ background: "#16233A", border: "1px solid #2A3B57" }}>
              {fixedRatio !== null && (
              ...
              <p className="text-sm">총 지출 <span className="tabular font-bold">{won(grandTotal)}</span></p>
            </div>
          )}
                ...
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={pieData} dataKey="total" nameKey="name" innerRadius={50} outerRadius={85} paddingAngle={2}>
                      {pieData.map((entry) => (
                        <Cell key={entry.id} fill={entry.color} stroke="#16233A" strokeWidth={2} />
                      ))}
                    </Pie>
                    <Tooltip
                      formatter={(v, n) => [won(v), n]}
                      contentStyle={{ background: "#101B2D", border: "1px solid #2A3B57", borderRadius: 8, color: "#EDE6D3" }}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="flex flex-wrap gap-2 mt-2 justify-center">
                {pieData.map((c) => (
                  <span key={c.id} className="text-xs px-2 py-1 rounded-full flex items-center gap-1" style={{ background: "#101B2D", color: c.color }}>
                    <span className="w-2 h-2 rounded-full" style={{ background: c.color }} /> {c.emoji} {c.name} {won(c.total)}
                  </span>
                ))}
              </div>
            </div>
          ...
          {/* Ledger table */}
          <div className="rounded-2xl overflow-hidden" style={{ background: "#16233A", border: "1px solid #2A3B57" }}>
