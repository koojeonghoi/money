...
이미지 분석은 서버(Vercel 서버리스 함수)를 통해 Anthropic API를 호출하므로, API 키가 브라우저에 노출되지 않습니다.

## 1. 준비물

- [Anthropic Console](https://console.anthropic.com)에서 발급받은 API 키 (`sk-ant-...`)
- GitHub 계정
...
모든 거래 내역과 카테고리 설정은 브라우저의 `localStorage`에 저장됩니다. 서버로 전송되거나 별도로 기록되지 않으며, 이미지 분석 요청 시 이미지 자체만 Anthropic API로 전송됩니다.

## 6. 사용법

1. 카드 명세서나 가계부 앱 캡처를 화면에 붙여넣기(Ctrl/Cmd+V)
2. 자동으로 인식된 내역과 카테고리를 확인, 필요하면 직접 수정
3. 카테고리는 좌측 "카테고리 편집"에서 이름/색상/고정·변동·저축 구분을 자유롭게 편집
